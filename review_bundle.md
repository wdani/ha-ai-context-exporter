...
- Neuer read-only Download-Endpoint `GET /api/export/download` implementiert.
- Download nutzt dieselbe zentrale Exportquelle wie `/api/export` (`build_export_payload(...)`), ohne doppelte Exportlogik.
- Unterstützte Download-Formate: `json` und `markdown` (inkl. Validierung und HTTP-400 bei ungültigem Format).
- Renderer-Modul `export_renderers.py` ergänzt (JSON/Markdown-Rendering + Dateiname auf Basis von `generated_at`).
...
- In diesem Schritt wird bewusst nur `json` und `markdown` unterstützt; `zip` ist absichtlich noch nicht implementiert.
- Der Dateiname nutzt den bestehenden `generated_at`-Wert aus der Payload; der Timestamp wird dateisystemfreundlich normalisiert (z. B. `:` -> `-`).
- Markdown-Ausgabe ist generisch rekursiv für verschachtelte Dict/List-Strukturen, bleibt aber absichtlich einfach gehalten.
