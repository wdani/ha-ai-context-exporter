...
- Primäre Authentifizierung auf offiziellen Home Assistant Core Proxy umgestellt (`homeassistant_api: true`, Requests an `http://supervisor/core/api/...`).
- Zentrale Header-Logik nutzt `SUPERVISOR_TOKEN` nur für erlaubte Core-Proxy-URLs; kein Token für Supervisor-API außerhalb `/core/` oder externe Hosts.
- Neuer token-sicherer Debug-Endpunkt `/api/ha-auth-debug` ergänzt (Reachability/Readability/HTTP-Status/Reason ohne Token-Leak).
- Bestehende Discovery-/Export-Struktur beibehalten und um proxy-orientierte Diagnosereasons (z. B. `core proxy unauthorized`) geschärft.
- Version auf `0.0.4` erhöht (Single Source of Truth in `version.py`, konsistent in `config.yaml`).
...
### `ha_ai_context_exporter/rootfs/app/export/export_system.py`
```python
...
### `ha_ai_context_exporter/rootfs/app/export/export_logic.py`
```python
...
### `ha_ai_context_exporter/rootfs/app/export/export_dashboard.py`
```python
...
- In der aktuellen Testumgebung lieferte der Core Proxy HTTP 403; dadurch bleiben Exportdaten ehrlich als nicht lesbar markiert, obwohl Reachability gegeben ist.
- Es wurden keine neuen Datenquellen, keine UI-Änderungen und keine Schreiboperationen im Anwendungscode eingeführt.
- `warnings` bleibt weiterhin stabil als Liste von Strings.