...
- Version auf **0.0.5** erhöht und konsistent in zentraler Version + `config.yaml` verwendet.
- Core-Proxy-Basis explizit gemacht (`CORE_API_BASE_CANDIDATES` mit `/api`-Basis) und Endpoint-Pfade auf relative API-Pfade umgestellt (verhindert `/api/api`-Fehlerbilder).
- Zentralen Request-Timeout auf 5 Sekunden erhöht, um Reachability/Readability robuster zu bewerten.
- Semantik für Bereichsstatus verbessert: `available` / `partial` / `unavailable` jetzt konsistenter für `system`, `areas_devices`, `dashboard`.
- `/api/ha-auth-debug` bleibt token-sicher und liefert klare Core-Proxy-Diagnose ohne Token-Leak.
...
- In der aktuellen Laufumgebung antwortet der Core Proxy mit HTTP 403; dadurch bleiben manche Bereiche korrekt als nicht lesbar markiert. Die verbesserte Semantik (inkl. `partial`) wurde zusätzlich strukturell mit direkten Funktionsaufrufen verifiziert.
- Keine UI-Änderungen in diesem Schritt; Fokus ausschließlich auf Discovery-Abdeckung, Semantik und Robustheit.
- `warnings` bleibt weiterhin eine Liste von Strings.